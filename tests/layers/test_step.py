...
def test_step_holds_pages_only(idle_driver: ChromeDriver) -> None:
    with pytest.raises(LayerError, match="may only hold page"):
        StepWithData(idle_driver, BASE)


def test_page_is_built_only_in_step_constructor(idle_driver: ChromeDriver) -> None:
...
def test_component_is_not_built_in_step(idle_driver: ChromeDriver) -> None:
    with pytest.raises(LayerError, match="must be created in a page constructor"):
        StepBuildingComponent(idle_driver, BASE)


def test_step_cannot_touch_elements(idle_driver: ChromeDriver) -> None:
...
def test_step_reads_state_through_pages(driver: ChromeDriver) -> None:
