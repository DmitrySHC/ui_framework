...
def test_group_holds_steps(idle_driver: ChromeDriver) -> None:
    steps = Steps(idle_driver, BASE)
    assert isinstance(steps.widgets, WidgetSteps)


def test_group_holds_nested_groups(idle_driver: ChromeDriver) -> None:
...
def test_asserts_group_holds_asserts(idle_driver: ChromeDriver) -> None:
    asserts = Asserts(idle_driver, BASE)
    assert isinstance(asserts.widgets, WidgetAsserts)


def test_nested_steps_group_rejects_asserts(idle_driver: ChromeDriver) -> None:
    with pytest.raises(LayerError, match="root steps group"):
        NestedWithAssert(idle_driver, BASE)


def test_root_steps_group_holds_asserts(idle_driver: ChromeDriver) -> None:
    group = StepsWithAssert(idle_driver, BASE)
    assert isinstance(group.widgets, WidgetAsserts)


def test_asserts_group_rejects_plain_value(idle_driver: ChromeDriver) -> None:
